...
const BookmarkBar: React.FC<BookmarkBarProps> = ({ bookmarks, onUpdateBookmarks }) => {
  ...
  return (
    <div className="w-full bg-white border-b border-slate-200 py-2.5 px-4 overflow-hidden">
      <div
        className="grid grid-cols-5 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-[repeat(15,minmax(0,1fr))] gap-1.5"
      >
        {bookmarks.slice(0, 15).map((bookmark) => (
          <div
            key={bookmark.id}
            className={`relative group/btn transition-all ${draggedBookmarkId === bookmark.id ? 'opacity-50' : ''
